...
def reply(signalCodeResponse, itemCode, seqNbr, result, scaledNetWeight):
    ...
    if "RSP" in signalCodeResponse:
        msgdtl["SignalData"]["TransactionResult"] = True
    else:
...
def callback(ch, method, properties, body):
    ...
    msg = json.loads(body)
    if msg["TransactionResult"] == False:
        signalCodeResponse = msg["SignalCode"]
    else:
